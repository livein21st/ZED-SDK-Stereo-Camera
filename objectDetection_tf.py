    ...
    while not exit_signal:
        if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
            ...
            zed.retrieve_measure(
                depth_mat, sl.MEASURE.XYZRGBA, resolution=image_size)
            lock.acquire()
    ...
    for i in range(num_detections):
        if scores_[i] > confidence:
            ...
            if len(x_vect) > 0:
                ...
                box_to_color_map[box] = vis_util.STANDARD_COLORS[classes_[
                    i] % len(vis_util.STANDARD_COLORS)]
...
def main(args):
    ...
    capture_thread = Thread(target=capture_thread_func, kwargs={
